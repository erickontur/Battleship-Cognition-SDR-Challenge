(() => {
  ...
  // ---------- Game State ----------
  /**
   * A ship has: id, name, size, cells: [{r,c}], hits: Set of "r,c"
   * A board has: grid[r][c] = { shipId | null, shot: bool }
  ...
  const state = {
    ...
    ai: {
      ...
      targets: [],            // queue of {r,c} to try next
      hits: [],               // confirmed hits on the current targeted ship
      lastShot: null,
    },
    lastPlayerShot: null,
    lastEnemyShot: null,
  ...
  function renderFleetPicker() {
    ...
    // Auto-select next unplaced ship if none selected
  ...
  function onEnemyCellClick(e) {
    ...
    fireAt(state.enemyBoard, state.enemyShips, r, c, /*isPlayerShot=*/ true);
  }
  ...
  function fireAt(board, ships, r, c, isPlayerShot) {
    ...
    // Clear previous last-shot indicator
    ...
    // In classic Battleship, turns alternate regardless of hit. That's what we use.
    if (isPlayerShot) {
  ...
  function enemyTurn() {
    ...
    fireAt(state.playerBoard, state.playerShips, r, c, /*isPlayerShot=*/ false);

    // Update AI brain based on result
    ...
    if (shipId) {
      ...
      if (ship.hits.size === ship.size) {
        // Ship sunk — clear target queue and go back to hunting.
        state.ai.mode = "hunt";
        state.ai.targets = [];
        state.ai.hits = [];
      } else {
      ...
      // Miss - stay in current mode; if targeting, keep draining the queue
  ...
  function chooseAIShot() {
    // Drain targets queue if targeting
    ...
    state.ai.mode = "hunt";
    // Hunt with parity (checkerboard with step=2, since smallest ship is 2)
    const candidates = [];
    for (let r = 0; r < BOARD_SIZE; r++) {
      for (let c = 0; c < BOARD_SIZE; c++) {
  ...
  function queueTargetsAroundHits() {
    ...
    if (hits.length === 1) {
      // Queue the 4 neighbors of the single hit
      const { r, c } = hits[0];
      const neighbors = [
        { r: r - 1, c },
        { r: r + 1, c },
    ...
    // 2+ hits: determine line and extend both ends
    ...
    if (horizontal) {
      ...
      // Hits are not collinear (rare when a second ship is adjacent). Fall back to neighbors of most recent hit.
  ...
  function endGame(playerWon) {
    state.phase = "over";
    el.enemyBoard.classList.remove("interactive");
    // Reveal any remaining enemy ships
